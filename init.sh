...
sed -i "s|ACCESS_ALLOWED_CIDR|$ACCESS_ALLOWED_CIDR|g" /etc/freeradius/3.0/clients.conf
sed -i "s|SHARED_SECRET|$SHARED_SECRET|g" /etc/freeradius/3.0/clients.conf

sed -i "s|BASE_DOMAIN|$BASE_DOMAIN|g" /etc/freeradius/3.0/proxy.conf
sed -i "s|DOMAIN_EXTENSION|$DOMAIN_EXTENSION|g" /etc/freeradius/3.0/proxy.conf

sed -i "s|GOOGLE_LDAP_PASSWORD|$GOOGLE_LDAP_PASSWORD|g" /etc/freeradius/3.0/mods-available/ldap
sed -i "s|GOOGLE_LDAP_USERNAME|$GOOGLE_LDAP_USERNAME|g" /etc/freeradius/3.0/mods-available/ldap

# add support to second level like: .com.br, .com.ar
sed -i "s|BASE_DOMAIN|$BASE_DOMAIN|g" /etc/freeradius/3.0/mods-available/ldap
if [[ ${DOMAIN_EXTENSION} =~ [.] ]]; then
    DOMAIN_EXTENSION=$( echo $DOMAIN_EXTENSION | awk -F'.' '{print $1",dc="$2}' )
fi
sed -i "s|DOMAIN_EXTENSION|$DOMAIN_EXTENSION|g" /etc/freeradius/3.0/mods-available/ldap

# Handle the certs
cp /certs/ldap-client.key /etc/freeradius/3.0/certs/ldap-client.key
cp /certs/ldap-client.crt /etc/freeradius/3.0/certs/ldap-client.crt
...
do
	if [ -f "/certs/$i" ]; then
	    cp /certs/$i /etc/freeradius/3.0certs/$i
	    chmod 644 /etc/freeradius/3.0/certs/$i
	fi
done
